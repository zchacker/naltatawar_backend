...
class PaymentsController extends Controller
{
    ...
    public function payments(Request $request)
    {
        ...
        $query      = PaymentsModel::where(['user_id' => $request->user()->parent()->id , 'status' => 'paid']);
        $sum        = $query->count('id');
        $contacts   = $query->paginate(100);
        
        return view('agent.payments.list', compact('contacts', 'sum'));
    }
    ...
    {
        $payment_data   = PaymentsModel::where(['user_id' =>  $request->user()->parent()->id , 'id' => $request->id ])->first();
        ...
        return view('agent.payments.invoice' , compact('formattedDate' , 'payment_data' , 'displayQRCodeAsBase64'));
    }
